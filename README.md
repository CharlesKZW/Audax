...
| Codex | `gpt-5.5` | `xhigh` | JSON validated against a schema |
...
in `audax_core/backends.py`; when entering an interactive mission prompt, type
`/agents` or `/models` to inspect the active agent runtime details.
...
Audax prints a compact startup card, waits for your prompt, and starts after
you press `Ctrl-D`. In an interactive terminal, type `/flags` to inspect or
change session settings, `/agents` or `/models` to inspect agent runtime
details, and `/help` to list startup commands before submitting the mission.
Startup commands include `/mode [direct|mission-spec]`, `/spec-rounds <n>`,
`/implementation-rounds <n>`, `/require-approval [on|off]`,
`/auto-commit [on|off]`, `/session-branch [on|off]`, and `/exit`.
...
- Codex reviews for bugs, missing requirements, repo-policy gaps, and test
  gaps.
- For web app missions, Codex should also validate critical browser flows
  with end-to-end Playwright checks when feasible rather than relying only on
  static inspection or lower-level test output.
